...
def show_contacts(book: dict, message: str):
    ...
    if book:
        for i, contact in book.items():
            print(
                f"{i:>3}.{contact[0]:<30} {contact[1]:<20} {contact[2]:<30} {contact[3]:<20}"
            )
    else:
        print(message)
    print("=" * 110 + "\n")
...
def change_contact(book: dict, cid: int):
    ...
    ch = []
    for item in [
        "Введите новое имя (или оставьте пустым , чтобы не изменять): ",
    ...
    match choice:
        ...
        case 4:
            ...
            for item in (
                ...
                "Введите коментарий для контакта: ",
                "Введите почту контакта: ",
            ):
